...
- rulebook file missing, unreadable, or invalid CSV
...
| `com.tracefinder.parser` | `LogParser` — turns raw text lines into `LogEntry` or records them as malformed. Knows nothing about severity or rules. |
...
| `com.tracefinder` (root) | `Main` — orchestrates the pipeline; `FatalErrorException` — the single funnel for every "stop the tool" condition |
