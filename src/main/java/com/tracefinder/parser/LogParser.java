...
public class LogParser {
    ...
    public ParseResult parseFile(Path path) throws IOException, LogFileException {
        new LogFileReader().checkFileUsable(path);

        List<LogEntry> entries = new ArrayList<>();
        List<MalformedLine> malformedLines = new ArrayList<>();

        try (Utf8LineReader lineReader = new Utf8LineReader(path)) {
            Utf8LineReader.Line line;
            while ((line = lineReader.nextLine()) != null) {
                if (line.overLong) {
                    malformedLines.add(new MalformedLine(line.lineNumber, line.shortForm));
                } else {
                    LogEntry entry = parseLine(line.content, line.lineNumber);
                    if (entry != null) {
                        entries.add(entry);
                    } else {
                        malformedLines.add(new MalformedLine(line.lineNumber, line.content));
                    }
                }
            }
        }
